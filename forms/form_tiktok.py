...
class FormTikTokDesign():
    ...
    def show_video_info(self, image_url, title_url):
        ...
        # Botones Video/Audio
        self.right_panel = tk.Frame(self.video_options, bg=COLOR_CUERPO_PRINCIPAL)
        ...
        style = ttk.Style()
        style.configure("style.TRadiobutton", background=COLOR_CUERPO_PRINCIPAL)
        rb_mp4 = ttk.Radiobutton(self.right_panel, text="Video (MP4)", variable=self.formato_seleccionado, value="mp4", style="style.TRadiobutton")
        rb_mp4.pack(side=tk.TOP, padx=5, pady=5)
        rb_mp3 = ttk.Radiobutton(self.right_panel, text="Audio (MP3)", variable=self.formato_seleccionado, value="mp3", style="style.TRadiobutton")
        ...
        self.button_download = tk.Button(self.panel_principal, text="Descargar", command=self.download_video, bg="#4CAF50", fg="white", font=("Arial", 16), relief=tk.RAISED, bd=4, cursor="hand2", padx=20, pady=10)
        self.button_download.pack(side=tk.TOP, pady=20)
        ...
        if self.validate_url_youtube(url):
            try:
                ...
                if formato == "mp4":
                    ...
                    audio_file = video_stream.download(output_path=download_path)
                    # Convertir a MP3
                    audioclip = AudioFileClip(audio_file)
                    mp3_file = audio_file.replace("mp4", "mp3")
                    audioclip.write_audiofile(mp3_file)
                    audioclip.close()
                    os.remove(audio_file)  # Opcional: eliminar el archivo original
